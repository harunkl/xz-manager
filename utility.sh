...
parse_expiry_db() {
  ...
  awk '
    ...
    function toepoch(d,   cmd, out){
      d=trim(d)
      if(d ~ /^[0-9]{10,}$/){ return d+0 } # epoch seconds
      # expect YYYY-MM-DD
    ...
    /^[ \t]*$/ {next}
    {
      ...
      if(n>=2){
        ...
        if(n>=3){
          # type name date
          if(a[3] ~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ || a[3] ~ /^[0-9]{10,}$/){
            typ=trim(a[1]); name=trim(a[2]); dt=trim(a[3])
          } else if(a[2] ~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ || a[2] ~ /^[0-9]{10,}$/){
            # name date type?
            name=trim(a[1]); dt=trim(a[2]); typ=(n>=3?trim(a[3]):"")
          }
        }
...
cleanup_expired_now() {
  ...
  while IFS=$'\t' read -r name expepoch typ; do
