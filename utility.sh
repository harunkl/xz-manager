...
zivpn_remove_user_guess() {
  ...
  local tmp; tmp="$(mktemp)"
  jq --arg n "$name" '
    # Support actual ZIVPN structure used by this project:
    # .auth.config = ["user1", "user2", ...]
    if (.auth? and .auth.config? and (.auth.config | type) == "array") then
      .auth.config = (
        .auth.config
        | map(
            if type == "object" then
...
cleanup_expired_now() {
  ...
  local removed=0 kept=0 total=0
  local restart_xray_needed=0 restart_zivpn_needed=0
  ...
  while IFS=$'	' read -r name expepoch typ datestr; do
    ...
    # Normalize type casing so DB stays compatible with menu.sh/xzbot.py
    ...
    if [[ "$expepoch" =~ ^[0-9]+$ ]] && (( expepoch <= now )); then
      ...
      case "$typ" in
        ZIVPN)
          ...
          restart_zivpn_needed=1
          ;;
        VMESS|VLESS|TROJAN)
          xray_remove_user_by_email "$name" || true
          restart_xray_needed=1
          ;;
        *)
          # Unknown type: try both, but still remove from expiry db
          xray_remove_user_by_email "$name" || true
          zivpn_remove_user_guess "$name" || true
          restart_xray_needed=1
          restart_zivpn_needed=1
      ...
      removed=$((removed+1))
      # expired entry is intentionally not written back to DB
    else
      if [[ -n "${typ:-}" ]]; then
  ...
  (( restart_xray_needed == 1 )) && restart_xray || true
  (( restart_zivpn_needed == 1 )) && restart_zivpn || true
