...
wait_dpkg_lock(){
  ...
  while fuser /var/lib/dpkg/lock >/dev/null 2>&1      || fuser /var/lib/dpkg/lock-frontend >/dev/null 2>&1      || fuser /var/lib/apt/lists/lock >/dev/null 2>&1      || fuser /var/cache/apt/archives/lock >/dev/null 2>&1; do
    if (( waited == 0 )); then
      echo -e "${Y}[INFO]${NC} Menunggu proses apt/dpkg lain selesai..."
    fi
    if (( waited >= max_wait )); then
      die "Lock apt/dpkg masih aktif setelah ${max_wait} detik. Coba lagi beberapa saat lagi."
    fi
    sleep 3
    waited=$((waited + 3))
  done
...
configure_nginx_ws_proxy(){
  # memastikan nginx default config punya proxy /vless dan /vmess (untuk mode DOMAIN)
  if [[ -d /etc/nginx/sites-available ]]; then
    if ! grep -q "location /vless" /etc/nginx/sites-available/default 2>/dev/null; then
      # jika file default beda, tulis ulang yang kompatibel
      cat >/etc/nginx/sites-available/default <<'EOF'
server {
  listen 80 default_server;
  listen [::]:80 default_server;
...
install_acme_and_issue_cert(){
  local domain="$1"
  echo -e "${C}==> Issue SSL (Let's Encrypt via acme.sh) untuk domain: ${W}${domain}${NC}"

  # dependency untuk acme.sh standalone
  if command -v apt-get >/dev/null 2>&1; then
    apt_update
    wait_dpkg_lock
  ...
  # stop nginx supaya port 80 bisa dipakai standalone
  ...
  /root/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 || die "Gagal issue SSL. Pastikan port 80 terbuka dan domain sudah pointing."

  /root/.acme.sh/acme.sh --installcert -d "${domain}" \
    --ecc \
    --key-file "${XRAY_KEY}" \
    --fullchain-file "${XRAY_CERT}" \
    --reloadcmd "systemctl reload xray >/dev/null 2>&1 || systemctl restart xray >/dev/null 2>&1 || true" >/dev/null 2>&1 || die "Gagal install cert"

  chmod 600 "${XRAY_KEY}" >/dev/null 2>&1 || true
  chmod 644 "${XRAY_CERT}" >/dev/null 2>&1 || true

  # start nginx lagi (port 80)
...
write_xray_config_if_empty(){
  ...
  if [[ "$mode" == "1" ]]; then
    ...
    domain="$(read_domain)"
    install_nginx
    configure_nginx_ws_proxy  # /vless & /vmess di port 80
